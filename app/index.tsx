...
const TILE = (W - 40 - TILE_SPACING * (TILE_COLUMNS -1)) / TILE_COLUMNS;
...
export default function FolderScreen({ parentFolder }: { parentFolder: Folder | null }) {
  ...
  /* -----------------------------  RENDER  ----------------------------- */
  return (
    <ImageBackground source={{ uri: 'https://www.transparenttextures.com/patterns/wood-pattern.png' }} style={styles.container} imageStyle={{ opacity: 0.1 }}>
      <View style={[styles.headerContainer, { paddingTop: safeTop }]}>
        <View style={styles.headerTopRow}>
            <View style={styles.headerLeft}>
                {parentFolder && (
                <TouchableOpacity onPress={router.back} style={styles.backBtn}>
                    <Ionicons name="arrow-back-circle-outline" size={36} color="#4A4A4A" />
                </TouchableOpacity>
                )}
                <View>
            ...
            <View style={styles.headerActions}>
                <TouchableOpacity style={styles.actionBtn} onPress={() => setSortOpen(o => !o)}>
                    <MaterialIcons name="sort-by-alpha" size={26} color="#4A4A4A" />
                </TouchableOpacity>
        ...
        <FlatList
            ...
            renderItem={({ item }) => (
                ...
                >
                    <TouchableOpacity style={styles.menuBtn} onPress={e => openItemMenu(item, e.nativeEvent)} hitSlop={12}>
                        <MaterialCommunityIcons name="dots-horizontal" size={22} color="#8A8A8A" />
                    </TouchableOpacity>
                    <View style={styles.tileIcon}>
                        <Ionicons name={item.type === 'folder' ? "folder-open-outline" : "musical-notes-outline"} size={40} color={item.type === 'folder' ? "#D2B48C" : "#A0522D"} />
                    </View>
            ...
            ListEmptyComponent={
                <View style={styles.emptyContainer}>
                    <Ionicons name="file-tray-outline" size={60} color="#D3D3D3" />
                    <Text style={styles.emptyText}>Empty Shelf</Text>
                    <Text style={styles.emptySubtext}>Add a folder or import sheet music</Text>
                </View>
            }
        />
      </View>
      ...
      <Modal visible={createOpen} transparent animationType="fade" onRequestClose={() => setCreateOpen(false)}>
        <View style={styles.overlay}><View style={styles.modal}>
            ...
            <View style={styles.modalBtns}>
              ...
              <TouchableOpacity style={styles.modalCreateBtn} onPress={makeFolder}><Text style={styles.modalCreateText}>Create</Text></TouchableOpacity>
            </View>
        </View></View>
      </Modal>
        ...
        <Pressable style={styles.menuOverlay} onPress={() => setMenuState({ ...menuState, visible: false })}>
          <View style={[styles.itemMenu, { top: menuState.y, left: menuState.x - 120 }]}>
            ...
            <TouchableOpacity style={styles.itemMenuOption} onPress={() => Alert.alert('Delete Item', `Are you sure you want to delete "${menuState.item?.name}"?`, [{ text: 'Cancel' }, { text: 'Delete', style: 'destructive', onPress: () => removeItem(menuState.item!) }])}>
              <Text style={{ color: '#CD5C5C' }}>Delete</Text>
            </TouchableOpacity>
          </View>
        </Pressable>
      </Modal>
          ...
          <Pressable style={styles.moveModalContainer}>
            <View style={styles.moveHeader}>
                <TouchableOpacity onPress={navigateMoveBack} disabled={movePath.length === 1} style={styles.moveBackBtn}>
                    <Ionicons name="arrow-back" size={24} color={movePath.length === 1 ? '#ccc' : '#A0522D'}/>
                </TouchableOpacity>
            ...
            <FlatList
                ...
                renderItem={({item}) => (
                    <TouchableOpacity style={styles.moveDestination} onPress={() => navigateMoveForward(item)}>
                        <Ionicons name="folder-outline" size={22} color="#D2B48C" />
...
const styles = StyleSheet.create({
  ...
  subtitleText: { fontSize: 15, color: '#8A8A8A' },
  headerActions: { flexDirection: 'row', alignItems: 'center' },
  actionBtn: { backgroundColor: 'rgba(255,255,255,0.5)', padding: 10, borderRadius: 25, marginRight: 10 },
  addBtn: { backgroundColor: '#A0522D', borderRadius: 25, width: 44, height: 44, alignItems: 'center', justifyContent: 'center', elevation: 4, shadowColor: '#A0522D', shadowOpacity: 0.4, shadowRadius: 5, shadowOffset: { width: 0, height: 2 } },
  sortMenu: { position: 'absolute', top: 110, right: 70, backgroundColor: '#FFFDF5', borderRadius: 12, padding: 10, elevation: 8, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, zIndex: 1000 },
  ...
  grid: { paddingTop: 20, paddingBottom: 48, paddingHorizontal: 20 },
  tile: { 
    borderRadius: 18, 
    width: TILE, 
    height: TILE + 20, 
  ...
  menuBtn: { position: 'absolute', top: 6, right: 6, padding: 4, zIndex: 10, borderRadius: 15 },
  ...
  modal: { width: '90%', maxWidth: 400, backgroundColor: '#FFFDF5', borderRadius: 20, padding: 24, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, shadowOffset: { width: 0, height: 0 } },
  ...
  modalCreateBtn: { backgroundColor: '#A0522D', paddingHorizontal: 25, paddingVertical: 12, borderRadius: 12 },
  modalCreateText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  
  menuOverlay: { flex: 1 },
  itemMenu: { position: 'absolute', backgroundColor: '#FFFDF5', borderRadius: 12, paddingVertical: 8, width: 150, elevation: 8, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 8, shadowOffset: { width: 0, height: 4 } },
  itemMenuOption: { paddingVertical: 12, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center' },
  menuDivider: { height: 1, backgroundColor: '#f0f0f0' },
  moveModalContainer: { width: '90%', height: '65%', backgroundColor: '#FFFDF5', borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column' },
  moveHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: '#EAE7DC' },
  moveBackBtn: { padding: 5 },
  moveTitle: { fontSize: 17, fontWeight: '600', flex: 1, textAlign: 'center', marginHorizontal: 10 },
  moveList: { flex: 1 },
  moveDestination: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#EAE7DC' },
